...
//  into ClaudeIsland via the shared Unix socket.
//
...
struct CodexHookInstaller {
    ...
    private static let bridgeScript = hooksDir.appendingPathComponent("claude-island-codex-hook.py")
    ...
    private static func installHooksConfiguration() {
        ...
        for (event, config) in allEvents {
            ...
            let cleanedEntries = existingEntries.compactMap { removingClaudeIslandHooks(from: $0) }
    ...
    private static func isClaudeIslandHook(_ hook: [String: Any]) -> Bool {
        ...
        return command.contains("claude-island-codex-hook.py")
    }
